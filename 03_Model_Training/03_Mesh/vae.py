...
class TransformerEncoder(nn.Module):
    def __init__(self, window_size, num_subcarriers, z_dim, num_heads=8, num_layers=4, gru_hidden=128):
        super().__init__()
        self.embed_dim = 128
        # 1D-Conv로 지역적 특징 추출 (CSI amplitude 특성)
...
class FrequencyLoss(nn.Module):
    # FFT 주파수 도메인에서 magnitude 차이에 페널티
    # 고주파 튀는 값 억제에 직접적으로 효과적
    def forward(self, recon, target):
        recon_fft  = torch.fft.fft2(recon,  norm='ortho')
...
class GradientLoss(nn.Module):
    # 이미지 엣지(gradient) 차이에 페널티
    # 경계가 뭉개지거나 흐릿해지는 것 방지
    def forward(self, recon, target):
...
class VAE(L.LightningModule):
    def __init__(self, window_size, num_subcarriers, z_dim=128, lr=1e-3,
                 ...
                 beta_warmup_epochs=20, ema_decay=0.999, **kwargs):
        super().__init__()
        ...
        self.ema       = None  # on_fit_start에서 초기화

    def on_fit_start(self):
        # EMA: inference 때 평균 가중치 사용 -> 더 안정적인 결과
